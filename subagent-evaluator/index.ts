...
// === HARNESS CONSTANTS (harmonized with subagent extension) ===
...
// Run a single subagent task with harness
async function runSubagent(
  ...
  if (!canSpawnSubagent(currentDepth)) {
    return {
      ...
      diagnostics: `Maximum subagent depth (${MAX_SUBAGENT_DEPTH}) exceeded. Cannot spawn nested subagents.`,
  ...
  try {
    ...
    // Pass incremented depth to child process
    ...
    exitCode = await new Promise<number>((resolve) => {
      ...
      proc.on("close", (code) => {
        ...
        // If timed out, return 124
        if (wasAborted) resolve(124);
        else resolve(code ?? 0);
      });
    ...
    if (!output) {
      output = wasAborted
        ? `Subagent timed out after ${timeoutMs / 1000} seconds. No final assistant report was produced.`
        : `Subagent finished without a final assistant report.`;
    }
    
    return { output, diagnostics: stderr, usage, exitCode, timedOut: wasAborted || exitCode === 124, hadFinalOutput, errorMessage };
  } finally {
    if (tmpPromptPath) {
...
${session.workConstraints ? `
🧭 Work Constraints: ${session.workConstraints}
` : ""}
...
const EvaluatedSubagentParams = Type.Object({
  ...
  work_constraints: Type.Optional(Type.String({
    description: "Bounded execution contract: scope limits, time budget, stop condition, and partial-results instruction"
  })),
  timeout_seconds: Type.Optional(Type.Number({
    description: "Child subagent timeout in seconds (default 600)",
    default: 600,
    minimum: 30,
    maximum: 1800
  })),
...
export default function subagentEvaluatorExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "evaluated_subagent tasks should include bounded work_constraints: scope, time budget, stop condition, output format, and partial-results instruction",
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx): Promise<AgentToolResult> {
      // Check depth - prevent recursive calls from subagents
      const currentDepth = getCurrentDepth();
      if (currentDepth > 0) {
        return {
          content: [{
            type: "text",
            text: `Error: evaluated_subagent can only be called from the main agent (depth 0). ` +
                 `Current depth: ${currentDepth}. This prevents runaway recursion and ensures the evaluation loop ` +
                 `only happens at the top level.`
          }],
          error: true,
        };
      }
      ...
      const session: EvaluationSession = {
        ...
        workConstraints: params.work_constraints,
        timeoutMs: Math.min(Math.max(params.timeout_seconds ?? 600, 30), 1800) * 1000,
        iteration: 1,
      ...
      const delegatedTask = params.work_constraints
        ? `${params.task}\n\nDelegation constraints:\n${params.work_constraints}\n\nReturn partial findings before stopping if the task is incomplete.`
        : params.task;
      const result = await runSubagent(agent, delegatedTask, params.cwd ?? ctx.cwd, signal, currentDepth, session.timeoutMs);
      ...
      if (!result.hadFinalOutput || result.timedOut || result.exitCode !== 0) {
        evaluationSessions.delete(session.id);
        return {
          content: [{
            type: "text",
            text: [
              "The subagent did not produce a valid report.",
              `Status: ${result.timedOut ? "timed out" : `exit code ${result.exitCode}`}`,
              result.output,
              result.diagnostics ? `\nDiagnostics:\n${result.diagnostics}` : "",
              "Retry with narrower work_constraints or inspect partial progress in details."
            ].filter(Boolean).join("\n")
          }],
          details: {
            session_id: session.id,
...
${session.workConstraints ? `\nOriginal work constraints still apply:\n${session.workConstraints}\n` : ""}
Provide a focused response that addresses these gaps. Return partial findings before stopping if incomplete.
`;
      
      const currentDepth = getCurrentDepth();
      const result = await runSubagent(agent, followUpTask, ctx.cwd, signal, currentDepth, session.timeoutMs);
      ...
      if (!result.hadFinalOutput || result.timedOut || result.exitCode !== 0) {
        evaluationSessions.delete(session.id);
        return {
          content: [{ type: "text", text: `The follow-up subagent did not produce a valid report. ${result.output}${result.diagnostics ? `\n\nDiagnostics:\n${result.diagnostics}` : ""}` }],
          details: {
            session_id: session.id,
            iteration: session.iteration,
            max_iterations: session.maxIterations,
