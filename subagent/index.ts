...
function buildDelegatedTask(task: string, workConstraints?: string, outputFormat?: string): string {
	const sections = [task.trim()];
	if (workConstraints?.trim()) {
		sections.push([
			"Delegation constraints:",
			workConstraints.trim(),
			"Return partial findings before stopping if the task is incomplete.",
		].join("\n"));
	}
	if (outputFormat?.trim()) {
		sections.push(["Requested output format:", outputFormat.trim()].join("\n"));
	}
...
async function runSingleAgent(
	...
	// Check subagent depth to prevent runaway nesting
	...
	if (currentDepth >= MAX_SUBAGENT_DEPTH) {
		return {
			...
			exitCode: 1,
			messages: [],
			stderr: `Subagent depth limit exceeded (${currentDepth} >= ${MAX_SUBAGENT_DEPTH}). This prevents runaway nesting of subagent calls.`,
			diagnostics: `Subagent depth limit exceeded (${currentDepth} >= ${MAX_SUBAGENT_DEPTH}). This prevents runaway nesting of subagent calls.`,
			timedOut: false,
	...
	if (!agent) {
		...
		return {
			...
			diagnostics: `Unknown agent: "${agentName}". Available agents: ${available}.`,
			timedOut: false,
			timeoutMs: SUBAGENT_TIMEOUT_MS,
			hadFinalOutput: false,
			usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, contextTokens: 0, turns: 0 },
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			proc.on("close", (code) => {
				...
				resolve(currentResult.timedOut ? 124 : (code ?? 0));
			});

			proc.on("error", (err) => {
				if (timeoutId) clearTimeout(timeoutId);
				currentResult.stderr += `Subagent spawn error: ${err.message}\n`;
				resolve(1);
			});
			...
			timeoutId = setTimeout(() => {
				if (!proc.killed) {
					...
					currentResult.stderr += `Subagent timed out after ${SUBAGENT_TIMEOUT_MS / 1000 / 60} minutes. Terminating...\n`;
					proc.kill("SIGTERM");
					setTimeout(() => {
...
const TaskItem = Type.Object({
	...
	work_constraints: Type.Optional(Type.String({ description: "Bounded execution constraints: scope limits, time budget, stop condition, partial-results instruction" })),
	output_format: Type.Optional(Type.String({ description: "Requested output format for the subagent report" })),
...
const SubagentParams = Type.Object({
	...
	work_constraints: Type.Optional(Type.String({ description: "Bounded execution constraints for single mode: scope limits, time budget, stop condition, partial-results instruction" })),
	output_format: Type.Optional(Type.String({ description: "Requested output format for the subagent report" })),
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				const results = await mapWithConcurrencyLimit(params.tasks, MAX_CONCURRENCY, async (t, index) => {
					const result = await runSingleAgent(
						...
						agents,
						t.agent,
						buildDelegatedTask(t.task, t.work_constraints, t.output_format),
						t.cwd,
						undefined,
						signal,
			...
			if (params.agent && params.task) {
				const result = await runSingleAgent(
					...
					agents,
					params.agent,
					buildDelegatedTask(params.task, params.work_constraints, params.output_format),
					params.cwd,
					undefined,
					signal,
					onUpdate,
